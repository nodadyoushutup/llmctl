...
- [x] Do not regress existing Agent task or flowchart node execution paths.
- [x] Prevent node-level skill binding through both UI and backend validation.
- [x] Preserve deterministic skill resolution order for Agent-bound skills.
- [x] Ensure migration/backfill path for legacy node-bound skill data (if present).
- [x] Keep runtime/audit metadata explicit so resolved Agent skills are traceable per run.

## Definition of done

- [x] Data model supports Agent-level skill bindings as the only active binding model.
- [x] All node-level skill-binding write paths are removed or rejected server-side.
- [x] Runtime skill resolution for tasks and nodes uses Agent-bound skills only.
- [x] Agent UI/API supports assigning, reordering, and removing skills.
- [x] Legacy node-bound skill records are migrated or archived with no runtime ambiguity.
- [x] Automated tests cover regressions, validation rules, and runtime resolution behavior.
- [x] Sphinx and Read the Docs docs are updated to reflect Agent-only skill assignment.
...
- [x] Add compatibility handling for stale clients sending node-level skill payloads.
- [x] Add feature flag or guarded rollout path if needed for incremental deployment.
- [x] Add operational metrics/logging for rejected node-level skill writes.
- [x] Document rollback approach if migration uncovers ambiguous ownership.

Deliverables:
- [x] Controlled rollout with observable enforcement signals.

### Stage 5 execution log (2026-02-16)

- Added compatibility mode helpers in `services/integrations.py`:
  - `node_skill_binding_mode` normalization/resolution (`warn`/`reject`)
  - constants for explicit mode handling across runtime/web paths
- Implemented mode-aware node-skill write behavior in `web/views.py`:
  - `warn` mode accepts legacy node `skill_ids` request shape, ignores persistence, and returns deprecation metadata
  - `reject` mode returns explicit validation/deprecation errors for node `skill_ids` writes
  - applied to graph upsert path and legacy `/flowcharts/<id>/nodes/<id>/skills*` routes
- Added structured deprecation event logging in `web/views.py` for stale payload usage:
  - emits machine-readable JSON payload in warning logs with source, mode, flowchart/node ids, path/method, and counts
- Added runtime rollout control in settings UI:
  - new Runtime settings section + form to switch `node_skill_binding_mode` between `warn` and `reject`
  - new backend route `POST /settings/runtime/node-skill-binding` persists the mode to integration settings
- Rollback approach documented/implemented:
  - operators can immediately switch back to `warn` mode to restore compatibility for stale clients without schema rollback
  - legacy node-skill rows remain soft-deprecated source data for deterministic reprocessing if needed

## Stage 6 - Automated Testing

- [x] Unit tests:
  - [x] Agent binding CRUD + ordering constraints
  - [x] validation rejects node-level binding payloads
  - [x] runtime resolver returns Agent-bound skills only
- [x] Integration tests:
  - [x] Agent task execution uses Agent skills
  - [x] flowchart node execution inherits Agent skills
  - [x] legacy migration path produces deterministic results
- [x] Regression tests for previously working task/node flows.

Deliverables:
- [x] Test suite passing with Agent-only skill binding guarantees.

### Stage 6 execution log (2026-02-16)

- Updated Stage 3/4 runtime tests to pass `agent_id` in task-node payloads so flowchart execution resolves Agent-bound skills under the current runtime contract.
- Added Stage 5 web/API coverage for Agent skill binding CRUD + manual ordering:
  - attach (dedupe-safe), move (up/down), and detach semantics verified through `agent_skill_bindings` positions.
- Added Stage 6 migration coverage for Agent-only binding cutover:
  - deterministic/idempotent migration from `flowchart_node_skills` to `agent_skill_bindings`
  - archive behavior for unmapped legacy node-skill rows in `legacy_unmapped_node_skills`.
- Test runs completed:
  - `.venv/bin/python3 -m unittest app/llmctl-studio/tests/test_skills_stage3.py app/llmctl-studio/tests/test_skills_stage4.py app/llmctl-studio/tests/test_skills_stage5.py app/llmctl-studio/tests/test_skills_stage6.py`
  - `.venv/bin/python3 -m unittest app/llmctl-studio/tests/test_agent_role_markdown_stage4.py`
  - `.venv/bin/python3 -m unittest app.llmctl-studio.tests.test_flowchart_stage9.FlowchartStage9UnitTests`

## Stage 7 - Docs Updates

- [x] Update Sphinx docs for skill architecture and assignment workflow.
- [x] Update Read the Docs pages describing Agent and node configuration.
- [x] Update internal planning/docs index entries if new docs are added.
- [x] Add migration/operator notes for environments with legacy node-bound skills.

Deliverables:
- [x] Documentation reflects Agent-only skill binding model and migration behavior.

### Stage 7 execution log (2026-02-16)

- Added Agent-only skill binding documentation page:
  - `docs/sphinx/agent_skill_binding.rst`
  - covers Agent-scoped source-of-truth, assignment workflow, node compatibility modes, runtime resolution, and migration/operator guidance.
- Updated docs navigation/index entries to include the new page:
  - `docs/sphinx/index.rst`
- Updated related docs cross-links and runtime guidance:
  - `docs/sphinx/codex_skills.rst` (clarified separation and linked Agent binding doc)
  - `docs/sphinx/provider_runtime.rst` (documented `node_skill_binding_mode` behavior with link to Agent binding doc)
- Updated release notes:
  - `docs/sphinx/changelog.rst`
- Verified docs build:
  - `.venv/bin/python3 -m sphinx -b html docs/sphinx docs/sphinx/_build/html`
