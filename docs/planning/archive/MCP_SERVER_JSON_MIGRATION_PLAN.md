...
- [x] Acceptance criteria: all executed automated checks pass.
- [x] Follow-up: documented runtime baseline issue where `test_chat_runtime_stage8.py` fails during collection with `ModuleNotFoundError: rag.contracts` under pytest import-path resolution.
