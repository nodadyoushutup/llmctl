...
Required:
  --url <url>                 URL to capture (example: http://localhost:5055/)
...
Options:
  ...
  --out-dir <path>            Output directory (default: ${LLMCTL_STUDIO_DATA_DIR:-/app/data}/screenshots)
...
resolve_browser_command() {
  ...
  echo "Error: no Chromium-compatible browser command found (checked chromium-browser, chromium, google-chrome-stable, google-chrome)." >&2
  return 1
}
...
out_dir="${default_data_dir%/}/screenshots"
repo_root="$(pwd)"
...
filename="${timestamp}--${route_slug}--${state_slug}--${viewport}--${git_sha}--${hash6}.png"
