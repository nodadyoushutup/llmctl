...
def _as_vector_array(values: list[float]):
    if np is None:
        return values
...
class GeminiEmbeddingFunction:
    ...
    def _embed(self, values: Any = None, **kwargs) -> list[np.ndarray]:
        ...
        return [_as_vector_array(vector) for vector in vectors]
...
def call_chat_completion(config, messages: list[dict[str, str]]) -> str:
    ...
    if OpenAI is None:
        raise RuntimeError("OpenAI Python package is required for openai chat provider.")
