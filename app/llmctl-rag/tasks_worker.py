...
from google_drive_sync import DriveSyncStats, count_syncable_files, sync_folder
from ingest import _iter_files, get_collection, index_paths
from logging_utils import log_sink, submit_with_log_context
from source_file_states_store import (
...
from sources_store import list_sources, schedule_source_next_index, update_source_index
    ...
    try:
        with log_sink(_task_output_sink(task_id)):
            ...
            if source_id is not None:
                ...
                existing_meta = task_meta(current_task) if current_task else {}
                configured_order = existing_meta.get("source_order")
                parsed_order = [
                    sid for sid in _as_int_list(configured_order) if sid in source_by_id
                ]
                if parsed_order:
                    new_ids = [sid for sid in source_by_id if sid not in parsed_order]
                    ordered_source_ids = parsed_order + new_ids
                else:
