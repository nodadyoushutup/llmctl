...
from models import SourceFileState
...
def list_source_file_states(source_id: int) -> list[SourceFileState]:
    init_db()
    with session_scope() as session:
        stmt = (
            select(SourceFileState)
            .where(SourceFileState.source_id == source_id)
            .order_by(SourceFileState.path.asc())
        )
...
def delete_source_file_states(source_id: int, *, paths: list[str] | None = None) -> None:
    ...
    with session_scope() as session:
        if not paths:
            stmt = select(SourceFileState).where(SourceFileState.source_id == source_id)
        ...
        for chunk in _chunked_unique_paths(paths):
            if not chunk:
                continue
            stmt = select(SourceFileState).where(
                SourceFileState.source_id == source_id,
                SourceFileState.path.in_(chunk),
            )
            for row in session.execute(stmt).scalars().all():
                row.delete(session)
...
def upsert_source_file_states(source_id: int, states: list[SourceFileStateInput]) -> None:
    ...
    paths = list(by_path)
    with session_scope() as session:
        existing_by_path: dict[str, SourceFileState] = {}
        for chunk in _chunked_unique_paths(paths):
            if not chunk:
                continue
            stmt = select(SourceFileState).where(
...
def summarize_source_file_states(source_id: int) -> SourceFileStats:
    ...
    with session_scope() as session:
        stmt = (
            select(
                SourceFileState.doc_type,
                func.count(SourceFileState.id),
                func.coalesce(func.sum(SourceFileState.chunk_count), 0),
            )
            .where(
                SourceFileState.source_id == source_id,
                SourceFileState.indexed == 1,
            )
            .group_by(SourceFileState.doc_type)
        )
